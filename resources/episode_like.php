<?php 
    ...
    if(!isset($_SESSION['userid']) && empty($_SESSION['userid'])){
        ...
        if($check_already->rowCount() == 0){
            $insert = $db->query("INSERT into phtv_episode_likes SET system_user_id = '$system_user_id', episode_id = '$episode_id'");
            $likes = $likes + 1;
            ...
            if($check_desliked->rowCount() > 0){
                ...
                $delete = $db->query("DELETE FROM phtv_episode_unlike WHERE system_user_id = '$system_user_id' AND episode_id = '$episode_id'");
            }
            if($insert){
                $response['success'] = 'success';
                $response['message'] = "Episode Liked Successfully";
        ...
        if($check_already->rowCount() == 0){
            $insert = $db->query("INSERT into phtv_episode_likes SET user_id = '$user_id', episode_id = '$episode_id'");
            $likes = $likes + 1;
            $update = $db->query("UPDATE phtv_voltage_episode SET likes = '$likes' WHERE id = '$episode_id'");
            ...
            if($check_desliked->rowCount() > 0){
                $dislike = $dislike - 1;
                $update = $db->query("UPDATE phtv_voltage_episode SET dislike = '$dislike' WHERE id = '$episode_id'");
                $delete = $db->query("DELETE FROM phtv_episode_unlike WHERE user_id = '$user_id' AND episode_id = '$episode_id'");
            }
            if($insert){
                $response['success'] = 'success';
                $response['message'] = "Episode Liked Successfully";
