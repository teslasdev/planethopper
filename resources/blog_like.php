<?php 
    ...
    if(!isset($_SESSION['userid']) && empty($_SESSION['userid'])){
        ...
        if($check_already->rowCount() == 0){
            $insert = $db->query("INSERT into phtv_blog_likes SET system_user_id = '$system_user_id', blog_id = '$blog_id'");
            $likes = $likes + 1;
            ...
            if($check_desliked->rowCount() > 0){
                ...
                $delete = $db->query("DELETE FROM phtv_blog_unlike WHERE system_user_id = '$system_user_id' AND blog_id = '$blog_id'");
            }
            if($insert){
        ...
        if($check_already->rowCount() == 0){
            $insert = $db->query("INSERT into phtv_blog_likes SET user_id = '$user_id', blog_id = '$blog_id'");
            $likes = $likes + 1;
            $update = $db->query("UPDATE phtv_blog SET likes = '$likes' WHERE id = '$blog_id'");
            ...
            if($check_desliked->rowCount() > 0){
                $dislike = $dislike - 1;
                $update = $db->query("UPDATE phtv_blog SET dislikes = '$dislike' WHERE id = '$blog_id'");
                $delete = $db->query("DELETE FROM phtv_blog_unlike WHERE user_id = '$user_id' AND blog_id = '$blog_id'");
            }
            if($insert){
                $response['success'] = 'success';
                $response['message'] = "Blog Liked Successfully";
